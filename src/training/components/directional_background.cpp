...
namespace lfs::training {
    ...
    namespace {
        ...
        lfs::core::Tensor make_color_tensor(const std::array<float, 3>& color) {
            std::vector<float> values{
                clamp_unit_color(color[0]),
                clamp_unit_color(color[1]),
                clamp_unit_color(color[2]),
            };
            return lfs::core::Tensor::from_vector(
                       values,
                       lfs::core::TensorShape({3UL}),
                       lfs::core::Device::CPU)
                .cuda();
        }
        ...
        lfs::core::Tensor target_to_chw_float_cuda(lfs::core::Tensor target) {
            if (!target.is_valid() || target.ndim() != 3) {
                throw std::runtime_error("Directional background target expects RGB image tensor");
            }

            const bool is_chw = target.shape()[0] == 3;
            const bool is_hwc = target.shape()[2] == 3;
            if (!is_chw && !is_hwc) {
                throw std::runtime_error("Directional background target expects CHW or HWC RGB image tensor");
            }

            const bool was_uint8 = target.dtype() == lfs::core::DataType::UInt8;
            if (target.dtype() != lfs::core::DataType::Float32) {
                target = target.to(lfs::core::DataType::Float32);
            }
            if (was_uint8) {
                target = target.div(255.0f);
            }
            if (is_hwc) {
                target = target.permute({2, 0, 1});
            }
            return contiguous_cuda(target);
        }
    } // namespace
    ...
    void DirectionalBackground::initialize(const std::array<float, 3>& color, const int degree, const std::array<float, 3>& up_axis) {
        ...
        fallback_color_cpu_ = {
            clamp_unit_color(learned_initial_color(color[0])),
            clamp_unit_color(learned_initial_color(color[1])),
            clamp_unit_color(learned_initial_color(color[2])),
        };
        fallback_color_ = make_color_tensor(fallback_color_cpu_);
    ...
    void DirectionalBackground::accumulate_target_gradient(
        const lfs::core::Camera& camera,
        ...
        if (!is_initialized() || loss_scale <= 0.0f) {
            return;
        }

        auto target = target_to_chw_float_cuda(target_image);
        const cudaStream_t stream = lobe_logits_.stream();
        target.set_stream(stream);
        const int height = static_cast<int>(target.shape()[1]);
        const int width = static_cast<int>(target.shape()[2]);
        const int pixel_count = std::max(1, height * width);

        auto rendered = render(camera, width, height, tile_x_offset, tile_y_offset, full_width, full_height);
        auto grad_image = rendered.sub(target).mul(loss_scale / static_cast<float>(pixel_count)).contiguous();
        grad_image.set_stream(stream);

        const lfs::core::TensorShape alpha_shape{
            static_cast<size_t>(height),
            static_cast<size_t>(width)};
        if (!alpha_zero_buffer_.is_valid() || alpha_zero_buffer_.shape() != alpha_shape) {
            alpha_zero_buffer_ = lfs::core::Tensor::zeros(alpha_shape, lfs::core::Device::CUDA);
        }
        alpha_zero_buffer_.set_stream(grad_image.stream());
    ...
    void DirectionalBackground::apply_sky_prefix_prior(
        lfs::core::SplatData& model,
        const lfs::core::Tensor& center,
        const int start,
        const int count,
        const float strength) const {
        if (!is_initialized() || strength <= 0.0f || count <= 0) {
            return;
        }
        if (!center.is_valid() || center.numel() < 3) {
            return;
        }
        if (!model.means().is_valid() || !model.sh0_raw().is_valid()) {
            return;
        }

        auto center_cuda = contiguous_cuda(center);
        const cudaStream_t stream = lobe_logits_.stream();
        center_cuda.set_stream(stream);
        lfs::training::kernels::launch_apply_directional_background_sky_prefix_prior(
            lobe_dirs_.ptr<float>(),
            lobe_logits_.ptr<float>(),
            fallback_color_.ptr<float>(),
            model.means().ptr<float>(),
            model.sh0_raw().ptr<float>(),
            center_cuda.ptr<float>(),
            lobe_count_,
            lobe_sharpness_for_degree(degree_),
            start,
            count,
    ...
    void DirectionalBackground::propagate_sky_prefix(
        lfs::core::SplatData& model,
        const lfs::core::Tensor& center,
        const int start,
        const int count,
        ...
        if (color_strength <= 0.0f && opacity_strength <= 0.0f && scale_strength <= 0.0f) {
            return;
        }
        if (!center.is_valid() || center.numel() < 3) {
            return;
        }
        if (!model.means().is_valid() ||
            !model.sh0_raw().is_valid() ||
            !model.opacity_raw().is_valid() ||
            !model.scaling_raw().is_valid()) {
            return;
        }

        const lfs::core::TensorShape stats_shape{
            static_cast<size_t>(lobe_count_),
            5UL};
        const lfs::core::TensorShape weights_shape{static_cast<size_t>(lobe_count_)};
        const cudaStream_t stream = lobe_logits_.stream();
        if (!sky_prefix_stats_.is_valid() || sky_prefix_stats_.shape() != stats_shape) {
            sky_prefix_stats_ = lfs::core::Tensor::empty(stats_shape, lfs::core::Device::CUDA);
        }
        if (!sky_prefix_weights_.is_valid() || sky_prefix_weights_.shape() != weights_shape) {
            sky_prefix_weights_ = lfs::core::Tensor::empty(weights_shape, lfs::core::Device::CUDA);
        }

        auto center_cuda = contiguous_cuda(center);
        center_cuda.set_stream(stream);
        sky_prefix_stats_.set_stream(stream);
        sky_prefix_weights_.set_stream(stream);
        sky_prefix_stats_.zero_();
        sky_prefix_weights_.zero_();

        const float clamped_min_opacity = std::clamp(min_opacity, 0.0f, 0.98f);
        const float max_log_scale = std::log(std::max(max_scale, 1.0e-6f));
        lfs::training::kernels::launch_accumulate_sky_prefix_lobe_stats(
            lobe_dirs_.ptr<float>(),
            model.means().ptr<float>(),
            model.sh0_raw().ptr<float>(),
            ...
            center_cuda.ptr<float>(),
            sky_prefix_stats_.ptr<float>(),
            sky_prefix_weights_.ptr<float>(),
            lobe_count_,
            lobe_sharpness_for_degree(degree_),
            start,
            count,
        ...
        lfs::training::kernels::launch_apply_sky_prefix_lobe_stats(
            lobe_dirs_.ptr<float>(),
            sky_prefix_stats_.ptr<float>(),
            sky_prefix_weights_.ptr<float>(),
            fallback_color_.ptr<float>(),
            ...
            center_cuda.ptr<float>(),
            lobe_count_,
            lobe_sharpness_for_degree(std::max(0, degree_ - 1)),
            start,
            count,
            color_strength,
            opacity_strength,
    ...
    void DirectionalBackground::deserialize(std::istream& is) {
        ...
        sky_prefix_stats_ = {};
        sky_prefix_weights_ = {};
        fallback_color_cpu_ = {0.82f, 0.88f, 0.96f};
        fallback_color_ = make_color_tensor(fallback_color_cpu_);
        render_width_ = 0;
        render_height_ = 0;
